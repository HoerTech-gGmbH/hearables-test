function accelerometer_only()
    close all;
    clear all;
    serialPortName='COM4';   %name of serial port connected to the headset
    ...
    while (stayinloop)
        ...
        if (~stayinloop)
            ...
            filenamePDF = sprintf('accelerometer_%s.pdf', datestr(now,'mm-dd-yyyy HH-MM'))
            print('-dpdf', filenamePDF);
            % add time and date here
            filenameCSV = sprintf('accelerometer_%s.csv', datestr(now,'mm-dd-yyyy HH-MM'))
            csvwriteWithHeader(filenameCSV, [(0:length(x)-1)'/calculatedFs x' y' z' status' (timestamps-timestamps(1))'], ...
