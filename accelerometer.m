function accelerometer()
    close all;
    clear all;
    serialPortName='COM3';   %name of serial port connected to the headset: find in device manager (Win + X --> Device Manager)

    %Beep Bop parameters
    beep_bop_interval=600;   %beep bop interval in ms
    beep_bop_index=[9 19];   %index positions with a deviation
    beep_bop_delta=[60 -60]; %deviation at index positions in degrees (360° is the period time)
    ...
    beepBopInit(beep_bop_interval, beep_bop_index, beep_bop_delta); %init beep bop parameters
    beepBopInterval(beep_bop_interval);%change interval time. Not needed but can also be changed during live measurement
    ...
    beepBopStart(); %start beep bop sounds
    %beepOnlyStart(); %start beep only sounds
    stayinloop=1;
    x=[];
    y=[];
    z=[];
...
%     setfigA4('portrait', 2);
    ...
    while (stayinloop)
        ...
        while (serialport.BytesAvailable>132) %enough data for new packet. 8 bytes timestamp + 3*20*2 acc bytes + 5 packet header
        ...
        if (~stayinloop)
            ...
            csvwriteWithHeader('accelerometer.csv', [(0:length(x)-1)'/calculatedFs x' y' z' status' (timestamps-timestamps(1))'], {'time[s]' 'x-acceleration [g]' 'y-acceleration [g]' 'z-acceleration [g]' 'status' 'timestamp[s]'});
        end
    end
    fclose(serialport);
    delete(serialport);
    ...
    function beepBopInit(beep_bop_interval, beep_bop_index, beep_bop_delta)
        sendPacket('D1', '0B', [mod(beep_bop_interval, 256) floor(beep_bop_interval/256) length(beep_bop_index) reshape([beep_bop_index;beep_bop_delta], 1, 2*length(beep_bop_index))]); %Beep bop init
    end

    function beepBopInterval(beep_bop_interval)
        sendPacket('D1', '0B', [mod(beep_bop_interval, 256) floor(beep_bop_interval/256)]); %Beep bop change interval. Can also be done while playing
    end

    function beepBopStart()
        sendPacket('D1', '0B', [1 0]); %beep bop start
    end

    function beepOnlyStart()
        sendPacket('D1', '0B', [2 0]); %beep only start
    end

    function beepBopStop()
        sendPacket('D1', '0B', [0 0]); %beep bop stop
    end
